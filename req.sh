...
echo "CREATE DATABASE $DATABASE_NAME;"|mysql -u $USER_NAME --password=$PASS
echo "GRANT ALL ON $DATABASE_NAME.* TO '$USER_NAME'@'localhost' IDENTIFIED BY '$PASS';"|mysql -u $USER_NAME --password=$PASS
echo "EXIT"|mysql -u $USER_NAME --password=$PASS
